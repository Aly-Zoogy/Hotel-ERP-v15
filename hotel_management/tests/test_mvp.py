...
def test_owner_settlement_calculation(owner_name, unit_name):
    ...
    try:
        ...
        expected_commission = revenue * 0.15
        owner_expenses = settlement.owner_share_expenses
        expected_net = revenue - expected_commission - owner_expenses
...
def test_settlement_methods(owner_name):
    ...
    try:
        # Cleanup any existing settlements for this owner to avoid duplicates
        existing_settlements = frappe.get_all("Owner Settlement", filters={"property_owner": owner_name})
        for s in existing_settlements:
            frappe.delete_doc("Owner Settlement", s.name, force=1)
        ...
        # Delete A to avoid duplicate name collision
